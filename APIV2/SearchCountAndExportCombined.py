...
with open('Relativity.json', 'r', encoding='utf-8-sig') as file:
    relatility = json.load(file)
...
selectedType.set("Relativity") # default value
typeMenu = OptionMenu(top, selectedType, "Ad1", "Native", "LoadFile", "ImageLoadFile", "ProductionSet", "Relativity") #, "BulkPrintJob")
...
def clickedSubmit():
    ...
    caseResp = requests.get(f'http://{servername}:4443/api/v2/enterpriseapi/core/getcaselist',headers = headers)
    jsonCaseResp = list(json.loads(caseResp.content))
    casePath = next((case for case in jsonCaseResp if str(case['id']) == str(caseId)), None)['casePath']
    reportPath = os.path.join(casePath, 'SearchReports')
    ...
    for lblId, lblName in labelIds:
        ...
        if tp == "Relativity":
            req = relatility
            req['exportBase']['exportPath'] = exportFolder 
            req['exportBase']['label'] = {'Id' : str(lblId)}
        else:
